...
The [approval-response protocol](approval-response.md) is mandatory before asking for acceptance, authorization, or approval and when processing the response. Before approval, report the exact changed surface, observable behavior, verification, unavailable evidence, what acceptance would authorize, what remains excluded, and the next bounded task. Approval never begins the next tranche automatically.

After an explicit affirmative, record the outcome when the established workflow requires it using `record add` and explicitly update current-state routing in the same authorized record task, including terminal closure. Provide the detailed copy-ready prompt for the next bounded task in a new session, and stop. Do not leave closure pending only because the terminal handoff lives in a conversation; persistence of the existing decision does not require another acceptance of that same closure. After a rejection or material revision request, leave the checkpoint pending and use the facilitated rejection interview, one material question per turn, until the reason, correction, evidence, and acceptance criteria are understood. Conditional and ambiguous responses remain subject to the canonical fail-closed classifications.
