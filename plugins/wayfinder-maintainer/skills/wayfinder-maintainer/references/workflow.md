...
Use one consolidated version-bound review question in chat. Review requests, feedback, and approval stay in chat; do not post PR review-request comments, factual tags, or review notifications. Finish all authorized persistence before sending the chat request, then stop without polling until the owner returns. On return, inspect feedback; explicit approval must identify the reviewed PRs/versions before eligible bottom-up integration under checks, protections, and any planned merge groups. Review completion alone does not permit merging. Native-stack unavailability requires a proposed fallback and decision, without installing or silently substituting tooling.
...
Use `maintain.py review`, `delivery`, and `verification` with the [local reliability CLI](../resources/plan-to-pr-development/reliability-cli.md). Records are immutable artifacts under each repository plan. New commands are local-only. Review requests, feedback, and approval are chat-only. Existing comment receipts remain historical and may be inspected within separately authorized reconciliation; they do not authorize new PR comments or tags. Missing required local coverage blocks review readiness; explicit plan-accepted environment limitations remain visible and never count as passes. Shared JSON envelopes and human summaries derive from the same structured results. An uncertain submission is saved as uncertain and stops delivery; reconcile before any resend on owner return.
