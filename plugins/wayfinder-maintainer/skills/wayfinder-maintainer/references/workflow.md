...
After an explicit affirmative, record the outcome when the established workflow requires it, provide the detailed copy-ready prompt for the next bounded task in a new session, and stop. After a rejection or material revision request, leave the checkpoint pending and use the facilitated rejection interview, one material question per turn, until the reason, correction, evidence, and acceptance criteria are understood. Conditional and ambiguous responses remain subject to the canonical fail-closed classifications.
