...
The owner instructed "Implement the proposed plan." for the complete Plan-to-PR Development proposal. The named authority in `wr-0035` covers its separate exact prerequisite PR, research resources, repository plan storage and CLI, governance/context integration, tests, continuous implementation, selective delegation, and PR delivery to `candidate-revision-9-certification`. Read the [approved plan](../../../../../docs/plans/development/wp-47a553ae-2a54-4dbe-a2dc-74c817521a08-plan-to-pr-development/approvals/revision-2.md) and current plan with `maintain.py plan read --id wp-47a553ae-2a54-4dbe-a2dc-74c817521a08 --history`. This later named instruction supersedes prior no-later-tranche and mandatory new-session boundaries only for this project. Implementation acceptance and merging remain pending owner review.

Approval covers task branches, commits, non-force pushes, PR creation/updates, applicable PR CI, and review requests. Pause for material revisions, insufficient authority, named checkpoints, or unavailable native-stack support. After requesting owner reviews, stop and do nothing until the owner returns; no polling, scheduled monitoring, auto-merge, or further implementation. On return, explicit owner approval bound to identified PRs/reviewed versions authorizes eligible bottom-up merges subject to checks, protections, and any plan merge groups. Review completion alone is not approval.
