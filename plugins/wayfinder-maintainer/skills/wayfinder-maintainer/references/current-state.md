...
The performance acceptance checkpoint remains paused after serial/two/four-worker jobs passed in 29m35s, 15m23s and 14m18s. The owner explicitly answered "yes, I approve" to complete diagnostic revision 8 in the isolated worktree (reviewed SHA-256 22335267a8c569ebb6da5e784e0d98d83cb7f66f885a4c4e718be2768b69ce66), preserved here as approved snapshot revision 9. This later named authority permits only the prepared startup/adapter-loading/real-command diagnostic checkpoint: commit and non-force push the isolated branch including template guidance and findings, update draft PR #9 with its template, run exactly one automatic ordinary validation attempt on the existing Ubuntu runner with 196 diagnostic process measurements and full 305-case-per-adapter coverage, inspect metadata/logs, persist findings locally, then pause for a measured plan-revision decision. No additional attempt is authorized after diagnostic failure without an explicit decision. Two PowerShell workers accompany this checkpoint; no final worker count is selected. Other adapter parallelism, sharding, reduced coverage, larger runners and adapter changes remain excluded. This is not implementation acceptance, readiness validation, review or merge authority. The owner checkout separately retains durable receipts and unrelated unpublished history, which remain excluded from the PR. Dependencies, certification/publication dispatch, accepted evidence publication/downloads, candidate reopening, activation, settings changes, main integration and unrelated work remain excluded. Embedded candidate/evidence/publication/activation facts are unchanged.
