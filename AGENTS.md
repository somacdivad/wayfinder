...
- Use [Plan-to-PR Development](plugins/wayfinder-maintainer/skills/wayfinder-maintainer/resources/plan-to-pr-development/README.md) for changes to either plugin. Discover repository plans with `maintain.py plan list`; read exact scope and approvals with `plan read --id ID --history`; create and update with `plan create/update --input FILE [--dry-run]`. Plans live under `docs/plans`, independently of plugin installation.
- Before requesting or processing an explicit acceptance, authorization, or approval, follow the maintainer [approval-response protocol](plugins/wayfinder-maintainer/skills/wayfinder-maintainer/references/approval-response.md). Within an explicitly approved development plan, an affirmative permits its named continuous implementation; a rejection requires the facilitated one-material-question-per-turn interview. Outside that plan, an affirmative requires the bounded new-session handoff and stop. Never begin the next task automatically outside the approved plan. At owner PR review handoff, use one consolidated version-bound approval question; save returned receipts or uncertain outcomes only when explicitly included in the approved terminal operation, then stop without polling until the owner returns.
...
Local `maintain.py review`, `delivery`, and `verification` commands manage immutable records beside repository plans; read the maintainer reliability CLI resource before use. Missing required local coverage blocks review readiness; explicit plan-accepted environment limitations remain unavailable/skipped. Local validation never infers owner approval or current platform eligibility.
